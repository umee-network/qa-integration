...
def execute_authz_tx(
    ...
    command = f"{DAEMON} tx authz exec {tx_file} --from {grantee_key} --chain-id {CHAINID} --keyring-backend test --home {DAEMON_HOME}-1 --node {RPC} --output json -y --gas {gas} -b block --fees {DEFAULT_FEES}"
...
def tx_grant_authz(
    ...
    command = f"{DAEMON} tx authz grant {grantee} send --spend-limit 100stake --from {granter} --chain-id {CHAINID} --keyring-backend test --home {DAEMON_HOME}-1 --node {RPC} --output json -y --gas {gas} -b block --fees {DEFAULT_FEES} --spend-limit 5uumee"
    return exec_command(command)
...
def tx_revoke_authz(
    ...
    command = f"{DAEMON} tx authz revoke {grantee} /cosmos.bank.v1beta1.MsgSend --from {granter} --chain-id {CHAINID} --keyring-backend test --home {DAEMON_HOME}-1 --node {RPC} --output json -y --gas {gas} -b block --fees {DEFAULT_FEES}"
    return exec_command(command)
